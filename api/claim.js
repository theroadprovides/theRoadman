...
// O token de participação agora é persistente.
// NÃO usamos mais TTL de 24h.
//
// O token é derivado de:
// PARTICIPATION_TOKEN_SECRET + wallet
//
// Portanto:
// mesma wallet = mesmo token.
...
//
// O servidor consegue reproduzir exatamente o mesmo
// token usando a mesma wallet + o mesmo segredo.
//
// Isso garante:
//
// wallet A -> token A
...
// wallet A -> token A
//
// sem precisar armazenar o token original.
//
...
//
// If the same wallet returns and the claim is still
// active/HOLDING, we return the existing Spot instead
// of creating another claim.
//
// The same wallet also reproduces the same participation
// token.
//
...
async function returnExistingClaim(
  ...
  // Make sure the participation_tokens table has
  // the persistent token hash associated with this claim.
  //
  // If it already exists, nothing is duplicated.
  const existingToken =
    await sql`
      SELECT
        id,
  ...
  if (
    ...
    // If an old/random token exists,
    // update it to the new deterministic token.
    //
    // This is necessary because the old implementation
    // generated random tokens that could not be recovered.
    if (
      stored.token_hash !==
      tokenHash
    ) {
  ...
  return {
    ...
    verification: {
      ...
      // This is the balance checked during
      // the current request.
      //
      // It is filled by the caller when appropriate.
      verified_balance:
        null,
    },
  };
...
module.exports =
  async function handler(
    ...
    try {
      ...
      //
      // IMPORTANT:
      // We search BEFORE checking for a new Spot.
      //
      // Same wallet + active claim =
      // same participant.
      //
      // No new Spot.
      // No new claim.
      ...
      if (
        ...
        if (
          ...
          // The wallet belongs to an existing participant.
          //
          // The X must also match.
          if (
            String(
              existing.x_handle
            ).toLowerCase() !==
          ...
          // We verified the wallet again during this request.
          let currentBalance;
          ...
          response.verification.verified_balance =
            currentBalance;
        ...
        // ---------------------------------------------
        //
        // Do NOT create another identity with the same
        // wallet.
        //
        // This preserves the participant history.
        // ---------------------------------------------
      ...
      // =================================================
      //
      // If the X already belongs to someone else,
      // don't create another Spot.
      // =================================================
      ...
      // =================================================
      // CREATE PERSISTENT PARTICIPATION TOKEN
      // =================================================
      ...
      try {
        ...
        console.error(
          "PARTICIPATION_TOKEN_ERROR",
          error
        );

        // The claim exists but its authentication
        // token was not created.
        //
        // We don't expose the claim to the client.
        return json(
          res,
          500,
          {
            success: false,
            error:
              "PARTICIPATION_TOKEN_CREATION_FAILED",
          }
        );
      }
